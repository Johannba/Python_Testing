class TestShowSummary:
    def test_valid_email_should_return_welcome_page(
        self, client, mock_clubs, captured_templates
    ):
        """
    ...
    def test_invalid_email_should_return_index_page_with_error(
        self, client, captured_templates
    ):
        """
        ...
        assert (
            "E-mail is unknown, please enter a valid e-mail !" in response.data.decode()
        )


class TestPurchasePlaces:
    def test_purchase_less_than_12_places(
    ...
    def test_purchase_more_than_points_places(
        self, client, mock_clubs, mock_competitions, captured_templates
    ):
        places_required = 13
        initial_points = int(mock_clubs[0]["points"])
...
class TestBookCompetition:
    def test_not_book_a_old_competition(
        self, client, mock_clubs, mock_competitions, captured_templates
    ):
        response = client.get(
            f"/book/{mock_competitions[1]['name']}/{mock_clubs[0]['name']}"
        )
        template, context = captured_templates[0]
        assert response.status_code == 200
        assert template.name == "welcome.html"
    ...
    def test_book_a_futur_competition(
        self, client, mock_clubs, mock_competitions, captured_templates
    ):
        response = client.get(
            f"/book/{mock_competitions[2]['name']}/{mock_clubs[0]['name']}"
        )
        template, context = captured_templates[0]
        assert response.status_code == 200
